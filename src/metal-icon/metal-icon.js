import { PropertyChangedHandler, PropertyChangedEventDispatcher, PropertyObserver, PropertyReflector } from 'html-element-mixins';

/**
 * Metal Elements icon
 * @element metal-icon
 * 
 * @cssprop --metal-icon-color
 * @cssprop --metal-icon-size
 * 
 */
...
export class MetalIconElement extends PropertyChangedEventDispatcher(PropertyChangedHandler(PropertyReflector(PropertyObserver(HTMLElement)))) { 
  ...
  _iconChanged() {
    ...
    if(!this.svgIcon) return this._render(null, '__empty');
    this._render(viewboxValue, svgIcon); 
  ...
  */  
  get svgIcon() {
    const ICONS = {
      ...
      `,
      'heart': `
      <g viewBox="0 0 43 43" fill="none" style="fill:var(--metal-icon-color, var(--metal-color-typography--icon))">
